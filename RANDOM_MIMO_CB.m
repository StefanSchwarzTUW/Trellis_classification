...
function CB = RANDOM_MIMO_CB(nRX,nTX,CB_size,seed,gauss,gains,corr_mat_sqrt,varargin)
    ...
    if isempty(varargin)
        for ii = 1:CB_size        
            if gauss      
                ...
                H = diag(gains,0)*1/sqrt(2)*corr_mat_sqrt*(randn(seed,nTX,nRX)+1i*randn(seed,nTX,nRX)); % "gains" accounts for channel gain differences between different transmit antennas
